...
- **Quantization**: AbsMax-style FP32 → ternary conversion and error metrics. CLI `quantize --input <path> --output <path>` loads a GGUF (F32 or I2_S weight tensors) and saves a BitNet GGUF (ternary). In-memory workflow is in `examples/quantize_model.rs`.
...
- **Phones / Termux**: Do **not** run HF→GGUF conversion on the device—it loads the full model into RAM and can crash all apps. Run conversion on a desktop or server and copy the `.gguf` file to the device. See [docs/termux_and_phones.md](docs/termux_and_phones.md).
